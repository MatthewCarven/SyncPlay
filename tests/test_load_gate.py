...
from syncplay.conductor import (
    LOAD_GATE_COLD,
    LOAD_GATE_TIMEOUT,
    STRAGGLER_GRACE_FRAC,
    hold_gate,
)

# The grace the caller derives for each kind of start. Tests use the cold one
# unless the warm figure is what's under test, so the numbers here are the ones
# the real fleet actually gets rather than an invented constant.
GRACE_COLD = LOAD_GATE_COLD * STRAGGLER_GRACE_FRAC      # 15.0 s
GRACE_WARM = LOAD_GATE_TIMEOUT * STRAGGLER_GRACE_FRAC   # 9.0 s
GRACE = GRACE_COLD


QUIET = GRACE + 1.0   # long enough that the grace has certainly gone
BUSY = GRACE - 1.0    # a node arrived recently enough to still count
...
def test_a_full_fleet_never_waits(quiet_for):
    ...
    assert hold_gate(grace=GRACE, n_ready=4, n_connected=4, quiet_for=quiet_for) is False
...
def test_an_empty_fleet_never_waits(quiet_for):
    ...
    assert hold_gate(grace=GRACE, n_ready=0, n_connected=0, quiet_for=quiet_for) is False


def test_more_ready_than_connected_is_not_a_deadlock():
    """A node can disconnect between the two counts being taken."""
    assert hold_gate(grace=GRACE, n_ready=3, n_connected=2, quiet_for=0.0) is False
...
def test_below_half_holds_however_quiet_it_gets(n_ready, n_connected, quiet_for):
    ...
    assert hold_gate(n_ready, n_connected, quiet_for, GRACE) is True


def test_exactly_half_is_enough_to_be_released_by_quiet():
    assert hold_gate(grace=GRACE, n_ready=2, n_connected=4, quiet_for=QUIET) is False
    assert hold_gate(grace=GRACE, n_ready=2, n_connected=4, quiet_for=BUSY) is True
...
def test_still_arriving_means_still_waiting():
    ...
    assert hold_gate(grace=GRACE, n_ready=3, n_connected=4, quiet_for=BUSY) is True


def test_gone_quiet_means_go():
    assert hold_gate(grace=GRACE, n_ready=3, n_connected=4, quiet_for=QUIET) is False
...
def test_the_grace_boundary():
    ...
    assert hold_gate(3, 4, GRACE - 1e-6, GRACE) is True
    assert hold_gate(3, 4, GRACE, GRACE) is False
...
def test_a_cached_node_cannot_strand_the_others():
    ...
    """
    assert hold_gate(grace=GRACE, n_ready=1, n_connected=4, quiet_for=3600.0) is True
...
def test_a_uniformly_slow_fleet_is_never_cut():
    ...
    for n_ready in (2, 3):
        assert hold_gate(n_ready, 4, 1.5, GRACE) is True
    assert hold_gate(4, 4, 1.5, GRACE) is False  # ...and releases on the last
...
def test_one_straggler_stops_the_drip():
    ...
    assert hold_gate(3, 4, 0.0, GRACE) is True
    assert hold_gate(3, 4, QUIET, GRACE) is False
...
def test_a_two_node_fleet_still_gets_bounded():
    ...
    assert hold_gate(1, 2, BUSY, GRACE) is True
    assert hold_gate(1, 2, QUIET, GRACE) is False


def test_a_lone_node_waits_for_itself():
    assert hold_gate(0, 1, QUIET, GRACE) is True
    assert hold_gate(1, 1, 0.0, GRACE) is False
...
def test_readiness_is_monotone(n_connected, quiet_for):
    ...
    decisions = [hold_gate(r, n_connected, quiet_for, GRACE) for r in range(n_connected + 1)]
...
def test_waiting_longer_is_monotone(n_connected):
    ...
    for n_ready in range(n_connected + 1):
        held = [hold_gate(n_ready, n_connected, q, GRACE) for q in (0.0, BUSY, QUIET, 3600.0)]
...
def test_a_full_fleet_always_releases_immediately(n_connected):
    assert hold_gate(n_connected, n_connected, 0.0, GRACE) is False


def test_the_grace_is_shorter_than_the_gate_it_sits_inside():
    """A quiet period at or past its own timeout is dead code — the timeout
    would always win first and the rule would never fire."""
    assert 0.0 < STRAGGLER_GRACE_FRAC < 1.0
    assert GRACE_WARM < LOAD_GATE_TIMEOUT
    assert GRACE_COLD < LOAD_GATE_COLD


def test_a_warm_start_is_less_patient_than_a_cold_one():
    """Warm starts interrupt music that was playing, so silence is felt; cold
    starts have already committed the room to a wait."""
    assert GRACE_WARM < GRACE_COLD
    # 10 s of quiet ends a warm wait but not a cold one.
    assert hold_gate(3, 4, 10.0, GRACE_WARM) is False
    assert hold_gate(3, 4, 10.0, GRACE_COLD) is True
